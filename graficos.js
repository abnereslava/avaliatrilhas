// =========================================================
// graficos.js - Módulo de Relatórios (Versão Expandida e Legível)
// =========================================================
...
(function() {
    ...
    function travarFiltrosSecundarios() {
        gEscopo.disabled = true; 
        gDisc.disabled = true; 
        gHab.disabled = true;
        gMesInicio.disabled = true; 
        gMesFim.disabled = true;
    ...
    function configurarCascata(selPolo, selTurno, selTurma, callback) {
        selPolo.addEventListener('change', () => {
            ...
            if (selPolo.id === 'g-polo-1') {
                ...
                gAluno1.disabled = true;
                travarFiltrosSecundarios();
            } else {
                gAluno2.innerHTML = '<option value="">Selecione o Outro Educando...</option>'; 
                gAluno2.disabled = true;
                btnGerar.disabled = true;
            }
            
            if (!selPolo.value) return;
            selTurno.appendChild(new Option("Manhã", "Manhã")); 
            selTurno.appendChild(new Option("Tarde", "Tarde"));
        ...
        selTurma.addEventListener('change', () => {
            if (selTurma.id === 'g-turma-1') {
                gAluno1.innerHTML = '<option value="">Selecione o Educando...</option>'; 
                gAluno1.disabled = true;
                if (!selTurma.value) travarFiltrosSecundarios();
            } else {
                gAluno2.innerHTML = '<option value="">Selecione o Outro Educando...</option>'; 
                gAluno2.disabled = true;
    ...
    tipoComp.addEventListener('change', () => {
        ...
        if (tipoComp.value === 'turma' && !gTurma2.value) {
            btnGerar.disabled = true;
        } else if (gTurma1.value) {
            btnGerar.disabled = false;
        }
    });
    ...
    gEscopo.addEventListener('change', () => {
        if (gEscopo.value === 'geral') { 
            ...
            areaDiscHab.style.display = 'flex'; 
            gDisc.style.display = 'block'; 
            gHab.style.display = 'none'; 
        } else { 
            areaDiscHab.style.display = 'flex'; 
            gDisc.style.display = 'block'; 
            gHab.style.display = 'block'; 
        }
    });
    ...
    async function carregarDadosTurma1() {
        ...
        try {
            ...
            gEscopo.disabled = false; 
            gDisc.disabled = false; 
            gHab.disabled = false;
            gMesInicio.disabled = false; 
            gMesFim.disabled = false; 
            ...
            console.error(e); 
            aviso.innerText = "❌ Erro ao conectar com o banco de dados."; 
        }
    }
    ...
    function gerarDatasetRadar(tipo, idAluno, ctxTurma, inicio, fim) {
        ...
        const disciplinas = Object.keys(bancoDeDados)
            ...
            .map(k => k.replace(`DISC_${ctxTurma}_`, ''))
            .sort();
    ...
    btnGerar.addEventListener('click', () => {
        ...
        if (escopo === 'geral') {
            ...
            if (comp === 'aluno') {
                ...
                if (!idAl2) return sysAlert("Selecione o outro educando.");
        ...
        if (objB) {
            datasets.push({ 
                ...
                data: objB.data, 
                borderColor: '#d35400', 
                backgroundColor: escopo==='geral' ? 'rgba(211, 84, 0, 0.2)' : 'transparent', 
                borderWidth: 3, 
                borderDash: escopo==='geral' ? [] : [5,5], 
                tension: 0.3, 
                fill: escopo==='geral', 
                pointRadius: 5, 
                spanGaps: true 
        ...
        chartInstance = new Chart(canvas.getContext('2d'), {
            ...
            options: {
                    ...
                    tooltip: { 
                        callbacks: { 
                            label: function(context) {
                                ...
                                let leg = ["0 (Não Aplicável)", "1 (Não Desenvolvida)", "2 (Em Desenvolvimento)", "3 (Em Consolidação)", "4 (Consolidada)", "5 (Destaque)"];
                ...
                scales: chartType === 'radar' ? { 
                    r: { min: 0.5, max: 5.2, ticks: { stepSize: 1, display: false } } 
                } : {
                    y: { 
                        min: 0.5, max: 5.2, ticks: { 
                            stepSize: 1, 
                            callback: function(v) {
                                if(v===1) return '1 - N.D.'; 
                                if(v===2) return '2 - E.D.'; 
                                if(v===3) return '3 - E.C.'; 
                                if(v===4) return '4 - Cons.'; 
                                if(v===5) return '5 - Dest.'; 
                                return null;
                            }
                        }
                    }
        ...
        // LEGENDA NA TELA COM AS CORES ATUALIZADAS (3 VERDE, 4 AZUL)
        ...
        legendaTela.innerHTML = `
            ...
            <span style="color: #9e9e9e; font-weight: bold;">0 (Não Aplicável)</span>
            <span style="color: #dc3545; font-weight: bold;">1 (Não Desenvolvida)</span>
            <span style="color: #d39e00; font-weight: bold;">2 (Em Desenvolvimento)</span>
            <span style="color: #28a745; font-weight: bold;">3 (Em Consolidação)</span>  <span style="color: #007bff; font-weight: bold;">4 (Consolidada)</span> <span style="color: #6f42c1; font-weight: bold;">5 (Destaque)</span>
        `;
        ...
        btnImprimirGrafico.addEventListener('click', () => {
            ...
            janelaImpressao.document.write(`
                ...
                <body>
                    ...
                    <div class="info-box">
                        <div><p><b>${subtitulo}</b></p><p><b>Análise:</b> ${escopoStr}</p>${detalheExtra}</div>
                        <div><p><b>${perStr}</b></p><p><b>Data de Emissão:</b> ${new Date().toLocaleDateString('pt-BR')}</p></div>
                    </div>
                    <img src="${canvasImg}">
                    <div class="legenda">
                        <span style="font-weight: bold; margin-right: 10px; color: #333;">Critérios:</span>
                        <span style="color: #9e9e9e; font-weight: bold;">0 (Não Aplicável)</span>
                        <span style="color: #dc3545; font-weight: bold;">1 (Não Desenvolvida)</span>
                        <span style="color: #d39e00; font-weight: bold;">2 (Em Desenvolvimento)</span>
                        <span style="color: #28a745; font-weight: bold;">3 (Em Consolidação)</span> <span style="color: #007bff; font-weight: bold;">4 (Consolidada)</span>                 <span style="color: #6f42c1; font-weight: bold;">5 (Destaque)</span>
    ...
    function resetarTelas() {
        if (telaTabela) telaTabela.style.display = 'none'; 
        if (telaGraficos) telaGraficos.style.display = 'none'; 
        if (telaLogs) telaLogs.style.display = 'none';
        
        if (btnVisaoTabela) btnVisaoTabela.classList.remove('visao-ativa'); 
        if (btnVisaoGraficos) btnVisaoGraficos.classList.remove('visao-ativa'); 
        if (btnVisaoLogs) btnVisaoLogs.classList.remove('visao-ativa');
    }
    ...
    if (btnVisaoGraficos) {
        btnVisaoGraficos.addEventListener('click', async () => {
            ...
            resetarTelas(); 
            btnVisaoGraficos.classList.add('visao-ativa'); 
            if (telaGraficos) telaGraficos.style.display = 'block';
    ...
    if (btnVisaoTabela) {
        btnVisaoTabela.addEventListener('click', () => { 
            resetarTelas(); 
            btnVisaoTabela.classList.add('visao-ativa'); 
            if (telaTabela) telaTabela.style.display = 'block'; 
        });
    }
})();
